...
// 圆形按钮
.sjc-button.is-circle {
  border-radius: 50%;
  padding: 12px;
}
// 让图标和文字之间空开
.sjc-button [class*="fa-"] + span {
  margin-left: 5px;
}
// 禁用
.sjc-button.sjc-button.is-disabled .mask {
  position: absolute;
  width: 100%;
  height: 100%;
...
.sjc-button.is-round .mask {
  border-radius: 20px;
}

.sjc-button.is-circle .mask {
  border-radius: 50%;
}
.sjc-button.is-disabled {
  cursor: no-drop;
}
</style>
